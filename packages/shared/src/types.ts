...
export type Stats = {
  ...
  charm: number;
  instinct: number;
  resolve: number; // resistance to manipulation: how hard they are to sway/betray
};
// Budget: 35 points total across 7 stats, each 1..8.
...
export type Contestant = {
  id: string;
  name: string;
  ownerPhone: string;      // PRIVATE: prize contact only, never leaves the server
  ...
  causeOfDeath: "combat" | "purge" | "weakestLink" | "voteOff" | null;  // ADDED (two-event decision)
...
export type PublicContestant = Omit<
  Contestant,
  "ownerPhone" | "ownerClientId" | "memory" | "nextThinkAt"
>;
...
export type Conversation = {
  ...
  // "tension" and "amicable" are the two soft outcomes added by the behavior
  // spec, sitting between "nothing" and the hard alliance/fight results. They
  // are additions: "truce" predates them and still resolves as it always has.
  outcome: "alliance" | "truce" | "fight" | "nothing" | "tension" | "amicable" | "ongoing";
...
export type Spectator = {
  ...
  name: string;
  phone: string;     // PRIVATE: never serialized to clients
  tokens: number;    // starts at 50
  notify: boolean;   // ADDED: opted in to SMS portfolio updates (default false)
  lastNotifiedAt?: number; // ADDED: rate-limit bookkeeping for SMS (server-only)
};

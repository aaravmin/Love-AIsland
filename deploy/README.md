...
The web app and sim server are deployed separately; the sim server runs behind Caddy for TLS under pm2 for auto-restart.
The browser talks to the droplet over `wss://`.

## What is actually deployed (as of Phase 3)

- **Server**: Ubuntu 24.04 with sufficient memory or swap for the Node process.
- **Public socket endpoint**: `wss://server.example.com` behind Caddy.
...
`NEXT_PUBLIC_SOCKET_URL=wss://server.example.com` is set for Production, Preview, and Development.
...
Code can be copied from a working tree with rsync:

```bash
rsync -az --exclude node_modules --exclude .next --exclude dist --exclude .git \
  packages/shared/ root@<server-ip>:/opt/love-aisland/packages/shared/
rsync -az --exclude node_modules --exclude .next --exclude dist --exclude .git \
  apps/server/ root@<server-ip>:/opt/love-aisland/apps/server/
rsync -az tsconfig.base.json root@<server-ip>:/opt/love-aisland/tsconfig.base.json
rsync -az apps/web/public/assets/island-map.json \
  root@<server-ip>:/opt/love-aisland/assets/island-map.json
```
...
- `curl https://server.example.com/healthz` returns `ok`.
